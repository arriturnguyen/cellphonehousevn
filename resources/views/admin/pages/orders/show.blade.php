...
<div class="row clearfix">
  ...
  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <div class="card">
      <div class="header">
        <h2>
        {{__('order.admin.show.detail_title')}}
        </h2>
      </div>
      <div class="body">
        <form id="form_advanced_validation" action="{{route('admin.orders.update', $order->id)}}" method="POST">
          ...
          <div class="form-group row clearfix">
            <div class="col-sm-4">
              ...
              <div class="form-line">
                <input name="submit_time" type="text" value="{{ $order->created_at }}" disabled="disabled" class="form-control time24">
              </div>
              </div>
            </div>
          </div>
          ...
          <div class="form-group row">
            <div class="col-sm-4">
              ...
              <select name="status" class="form-control">
                <option @if($order->status == 1) selected @endif value="1">{{ __('order.admin.message.status.pending')}}</option>
                <option @if($order->status == 2) selected @endif value="2">{{ __('order.admin.message.status.approved')}}</option>
                <option @if($order->status == 3) selected @endif value="3">{{ __('order.admin.message.status.shipped')}}</option>
                <option @if($order->status == 4) selected @endif value="4">{{ __('order.admin.message.status.cancel')}}</option>          
              </select>
            </div>
          </div>
